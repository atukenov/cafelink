...
import { ArrowLeft, Plus, Users, User, UserCheck } from 'lucide-react';
...
export default function AdminEmployeesPage() {
  ...
  const loadEmployees = async () => {
    try {
      ...
      setEmployees(data.filter((u: User) => ['employee', 'administrator'].includes(u.role)));
    } catch (err) {
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      case 'administrator': return <UserCheck className="w-5 h-5 text-green-600" />;
      ...
      <div className="max-w-md mx-auto p-4">
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">No employees yet</h3>
            <p className="text-gray-600">Add your first employee to get started</p>
          </div>
        ) : (
          <div className="space-y-3">
            {employees.map((employee) => (
              <div key={employee._id} className="bg-white rounded-xl shadow-sm p-4">
        ...
        <div className="mt-8 bg-blue-50 rounded-xl p-4">
          ...
          <div className="text-sm text-blue-600 space-y-1">
            <p><strong>Employee:</strong> Basic staff functions (orders, shifts, tasks)</p>
            <p><strong>Administrator:</strong> Employee + menu management</p>
          </div>
        </div>
      </div>
    </div>

...
async function createSecondShop() {
  try {
    ...
    // Create second shop
    const secondShop = await mongoose.connection.collection('coffeeshops').insertOne({
      name: 'Coffee Express',
      location: 'Astana',
    ...
    await mongoose.connection.collection('additionalitems').insertMany(additionalItems);
    ...
    await mongoose.connection.collection('loyaltyprograms').insertOne(loyaltyProgram);
    ...
    const employeeUser = await mongoose.connection.collection('users').insertOne({
      role: 'employee',
